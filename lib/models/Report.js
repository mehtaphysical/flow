...
schema.virtual('missingStudents').get(function() {
  return this.reports
    .filter(report => !report.report.avgTotalTests)
    .map(({ name }) => name);
});

schema.virtual('title').get(function() {
  return `Report ${format(this.createdAt, 'HH: dddd, mmmm d, yyyy')}`;
});
...
schema.methods.feedItem = function() {
  return {
    title: this.title,
    description: `Missing Students: ${this.missingStudents.join(', ')}`,
    content: `<p>
${this.toString()}
